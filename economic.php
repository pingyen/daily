<?php
	...
	$cachePath = __DIR__ . '/cache/economic' . ($time === null ? '' : "-$time") . '.html';
...
<link href="css/economic.min.css" rel="stylesheet" >
</head>
<body>
<header>
	<h1><a href="<?php echo $path ?>" >經濟部報紙新聞</a></h1>
...
<nav>
	<ul>
		...
		<li><a href="economic-word.php<?php if ($time !== null) { echo "?t=$time"; } ?>" >Word</a></li>
	</ul>
</nav>
<?php
	if ($time !== 'evening') {
...
<?php
	...
	foreach (json_decode(file_get_contents(__DIR__ . '/groups/economic' . ($time === null ? '' : "-$time") . '.json'), true) as $map) {
		++$count;
?>
	<article>
		<h3>新聞群組 <?php echo $count ?></h3>
