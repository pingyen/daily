<?php
	...
	$file = 'economic' . ($time === null ? '' : "-$time") . '.docx';
	...
	if (file_exists($cachePath) !== true || filemtime($cachePath) <= $mtime || (isset($_GET['c']) === true && intval($_GET['c']) === 0)) {
		...
		$section->addTitle('經濟部' . date(' Y 年 n 月 j 日', $mtime) . $label . '新聞', 'ttStyle');
		...
		foreach (json_decode(file_get_contents(__DIR__ . '/groups/economic-negative' . ($time === null ? '' : "-$time") . '.json'), true) as $map) {
			if ($count === 12) {
				break;
			}
		...
		$economic = json_decode(file_get_contents(__DIR__ . '/groups/economic' . ($time === null ? '' : "-$time") . '.json'), true);

		$section->addTitle('經濟部新聞', 'ttStyle2');
		$table = $section->addTable('tbStyle');

		foreach ($economic as $map) {
			$table->addRow(200);
			$cell = $table->addCell(1400, $cellStyle);
			$cell2 = $table->addCell(7200, $cellStyle);
		...
		$section->addTitle('經濟部新聞內文', 'ttStyle2');
		...
		$dept = json_decode(file_get_contents(__DIR__ . '/economic-dept.json', true));
